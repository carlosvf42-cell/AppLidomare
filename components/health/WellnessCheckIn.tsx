      ...
      <div className="max-w-[430px] w-full mx-auto flex flex-col h-full px-4 pt-14 pb-8">
                ...
                {q.opciones.map((opcion, idx) => {
                  ...
                  return (
                    <button key={idx} type="button" onClick={() => setAnswers((prev) => ({ ...prev, [q.key]: valor }))} className="flex-1 flex flex-col items-center gap-1 py-2 rounded-xl transition-all" style={{ background: selected ? "rgba(42,191,191,0.12)" : "rgba(255,255,255,0.04)", border: selected ? "0.5px solid rgba(42,191,191,0.4)" : "0.5px solid rgba(255,255,255,0.06)" }} title={opcion}>
                      ...
                      <span className="text-[10px] font-semibold" style={{ color: selected ? "#2abfbf" : "rgba(255,255,255,0.25)", fontFamily: "Barlow Condensed, sans-serif" }}>{valor}</span>
                    </button>
                  );
                })}
              </div>
              <div className="flex justify-between mt-1">
                <span className="text-[9px]" style={{ color: "rgba(255,255,255,0.2)", fontFamily: "Barlow Condensed, sans-serif" }}>{q.opciones[0]}</span>
                <span className="text-[9px]" style={{ color: "rgba(255,255,255,0.2)", fontFamily: "Barlow Condensed, sans-serif" }}>{q.opciones[4]}</span>
              </div>
            </div>
          ))}
        </div>
        {allDone && (
          <div className="rounded-2xl px-4 py-3 mb-3 flex items-center justify-between" style={{ background: `rgba(${puntuacion >= 20 ? "42,191,191" : puntuacion >= 13 ? "255,176,64" : "255,128,128"},0.08)`, border: `0.5px solid rgba(${puntuacion >= 20 ? "42,191,191" : puntuacion >= 13 ? "255,176,64" : "255,128,128"},0.25)` }}>
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.45)", fontFamily: "Barlow Condensed, sans-serif" }}>Estado general</p>
            <p className="text-sm font-semibold" style={{ color: getColor(puntuacion), fontFamily: "Barlow Condensed, sans-serif" }}>{puntuacion >= 20 ? "Bueno para entrenar" : puntuacion >= 13 ? "Entrenar con precaución" : "Considera descansar hoy"}</p>
          </div>
        )}
