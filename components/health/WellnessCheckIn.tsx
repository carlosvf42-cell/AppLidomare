      ...
      <div className="max-w-[430px] w-full mx-auto flex flex-col h-full px-4 pt-14 pb-8">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pb-4">
          {PREGUNTAS.map((q) => (
            <div key={q.key} className="rounded-2xl p-4" style={GLASS}>
              <p className="mb-0.5" style={{ fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(42,191,191,0.6)", fontFamily: "var(--font-ui)" }}>{q.label}</p>
              <p className="mb-3" style={{ fontSize: 14, color: "rgba(255,255,255,0.75)", fontFamily: "var(--font-ui)", letterSpacing: "0.03em", lineHeight: 1.4 }}>{q.pregunta}</p>
                ...
                {q.opciones.map((opcion, idx) => {
                  ...
                  return (
                    <button key={idx} type="button" onClick={() => setAnswers((prev) => ({ ...prev, [q.key]: valor }))} className="flex-1 flex flex-col items-center gap-1 py-2 rounded-xl transition-all" style={{ background: selected ? "rgba(42,191,191,0.12)" : "rgba(255,255,255,0.04)", border: selected ? "0.5px solid rgba(42,191,191,0.4)" : "0.5px solid rgba(255,255,255,0.06)" }} title={opcion}>
                      ...
                      <span className="text-[10px] font-semibold" style={{ color: selected ? "#2abfbf" : "rgba(255,255,255,0.25)", fontFamily: "var(--font-ui)" }}>{valor}</span>
                    </button>
                  );
                })}
              </div>
              <div className="flex justify-between mt-1">
                <span className="text-[9px]" style={{ color: "rgba(255,255,255,0.2)", fontFamily: "var(--font-ui)" }}>{q.opciones[0]}</span>
                <span className="text-[9px]" style={{ color: "rgba(255,255,255,0.2)", fontFamily: "var(--font-ui)" }}>{q.opciones[4]}</span>
              </div>
            </div>
          ))}
        </div>
        {allDone && (
          <div className="rounded-2xl px-4 py-3 mb-3 flex items-center justify-between" style={{ background: `rgba(${puntuacion >= 20 ? "42,191,191" : puntuacion >= 13 ? "255,176,64" : "255,128,128"},0.08)`, border: `0.5px solid rgba(${puntuacion >= 20 ? "42,191,191" : puntuacion >= 13 ? "255,176,64" : "255,128,128"},0.25)` }}>
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.45)", fontFamily: "var(--font-ui)" }}>Estado general</p>
            <p className="text-sm font-semibold" style={{ color: getColor(puntuacion), fontFamily: "var(--font-ui)" }}>{puntuacion >= 20 ? "Bueno para entrenar" : puntuacion >= 13 ? "Entrenar con precaución" : "Considera descansar hoy"}</p>
          </div>
        )}
        {error && <p className="text-xs text-center mb-2" style={{ color: "#ff8080" }}>{error}</p>}
        <button type="button" onClick={handleGuardar} disabled={!allDone || isSaving} className="w-full py-4 rounded-2xl text-sm font-semibold tracking-widest uppercase transition-all disabled:opacity-30" style={{ background: "#2abfbf", color: "#000", fontFamily: "var(--font-ui)", boxShadow: "0 4px 24px rgba(42,191,191,0.3)" }}>{isSaving ? "Guardando…" : ctaLabel}</button>
      </div>
    </div>
  );
}
