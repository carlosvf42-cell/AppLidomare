...
type RutinaDia = { id: string; nombre: string; orden: number; rutina_ejercicios: { id: string }[] };
...
export default function RutinasPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("rutinas")
      .select("id, nombre, activa, rutina_dias(id, nombre, orden, rutina_ejercicios(id))")
      ...
      <div className="px-4 pb-6">
        {loading ? (
          ...
          <div className="space-y-2">
            {rutinas.map((r) => {
              const dias = [...(r.rutina_dias ?? [])].sort((a, b) => a.orden - b.orden);
              const totalEjercicios = dias.reduce((s, d) => s + (d.rutina_ejercicios?.length ?? 0), 0);
              return (
                <button
                  key={r.id}
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      {dias.length > 0 && (
                        <div className="flex gap-1.5 mt-2 flex-wrap">
                          {dias.map((d) => (
                            <span key={d.id} className="text-[9px] px-2 py-0.5 rounded-full"
                    ...
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="shrink-0 mt-1">
